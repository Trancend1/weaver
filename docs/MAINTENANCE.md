...
```
Web smoke (no port binding): construct `create_api_app()` + FastAPI `TestClient` and assert `/health`·`/version`·`/projects` → 200 and the UI `/`→307·`/ui`·`/ui/new`·`/ui/providers` → 200.
...
- **Desktop sidecar PATH dependence — resolved** (Sprint P / ADR 016): the packaged app bundles a PyInstaller onedir sidecar via Tauri `externalBin`; resolver order `WEAVER_DESKTOP_SIDECAR` override → bundled → PATH fallback. The interactive desktop runtime smoke (launch → `/healthz` → hubs → close → no orphans → logs) remains a manual maintainer step before desktop-affecting releases.
...
**Expected skips (1 — not a regression):** `test_secret_store.py::…` POSIX file-mode (Windows host). Since v0.7.2 (ADR 018) the only live-provider test is `tests/integration/providers/test_openai_chat_live.py`, gated by `requires_cloud` / `requires_ollama` (deselected, not skipped, by the standard `-m "not requires_cloud and not requires_ollama"` run) — it needs a real key or a running OpenAI-compatible endpoint and is **not** run in CI by design. The old `test_deepseek_live.py` / `test_gemini_live.py` / `test_ollama_live.py` were removed with the native providers.
