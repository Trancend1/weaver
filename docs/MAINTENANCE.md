...
**Current baseline (Phase D — DOCX export, 2026-06-05):** `uv run pytest -q` → **716 passed / 4 skipped** (same expected skips; +13 DOCX renderer/service/API/UI tests over the Phase B6 703), pyright 0, ruff check + format clean, CLI 15 commands. **DOCX smoke:** `render_docx` → valid `.docx` ZIP with `[Content_Types].xml`, `_rels/.rels`, `word/{document,styles}.xml`, `word/_rels/document.xml.rels`; `POST /projects/{name}/export/novel {"target":"docx"}` → 202 → job `done` with one `.docx` artifact per volume under `output/docx/`. **QA smokes:** JSON `GET /projects/{name}/qa` (+ `…/volumes/{id}/qa`, `…/chapters/{id}/qa`) → 200 with `schema_version: 2`, `badge`, severity counts; UI `/ui/projects/{name}/qa` + `…/chapters/{id}/qa` → 200; export preflight `GET /ui/projects/{name}/export/preflight` → 200 (advisory, never blocks). QA is read-only/deterministic — a regression test asserts the QA path performs no DB write and calls no provider.
