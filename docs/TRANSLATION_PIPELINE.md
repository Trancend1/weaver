...
## 11. Export
Two export paths exist. The **legacy** single-source path (`services/export.py`) writes per-chapter **Markdown** review files and a translated **EPUB** (`renderers/epub.py`, xpath block rewrite + nav fallback) for a project's one configured `source_file`; it stays for the CLI `weaver export` and the Flask route.
