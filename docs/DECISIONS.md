...
| [020](decisions/020-bounded-translate-concurrency.md) | Bounded Translate Concurrency (**Proposed**) | Opt-in `[translation] max_concurrent = 1..4` (default 1 = today's behavior bit-for-bit): bounded in-process worker window inside the existing per-job thread, per-worker SQLite connections, locked cold-mark map, committed-only rolling window. Unlocked by the H3 transaction split. No asyncio/process pool/external queue. Gates v0.7.3 milestone M4; must be Accepted before implementation. |
