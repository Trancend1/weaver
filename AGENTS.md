...
- [ ] **Live validations — OPEN, owner-deferred 2026-07-26 (the only thing blocking the tag).** M5 did everything short of execution: the Gemini `requires_cloud` test and the first-ever `requires_ollama` test are written and collected, and the retired `gemini-1.5-flash` shim default is replaced with `gemini-2.5-flash`. Still unrun because the environment lacks the credentials/daemons: no `GEMINI_API_KEY` (shell env or `~/.weaver/secrets.toml`), no `ollama` binary and nothing on `:11434`, no `java`/`epubcheck`, no `py-spy`. Outstanding: (a) Gemini gated test passes live; (b) Ollama gated test passes live; (c) live concurrency spot-check ≥ 2× at `--max-concurrent 2` — **the 2.78× bench number does not count, it is simulated latency**; (d) E2 repair token-cost delta recorded → `enforce_repair` default confirmed. Both live tests deliberately **fail rather than skip** once opted into, so a green run is real evidence (CLAUDE.md §4.3 gate 5).
