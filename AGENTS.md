...
>
> **Current Sprint:** none active. After v0.7.3: **Cross-Platform Desktop (macOS/Linux)** with a fresh plan + ADR (§2.1.1); procure a code-signing cert when available to flip on signing.
...
| **v0.7.2 — Connection-First Routing + Enforcement Loop** | ✅ Done / tagged | ADR 018 + ADR 019, released as tag `v0.7.2` on `main`. **018:** one `openai_chat`(+`fake`), gemini/ollama natives + `google-generativeai` removed, connection registry + per-segment routing/fallback + discovery cache + CLI parity (S1–S5) + post-S5 hardening. **019:** enforcement loop + anti-slop (E1–E4). **Post-release audit (2026-07-05):** 3 High findings fixed on `fix/v072-audit-blockers` (key resolution, gemini shim URL, provider call inside write txn); Medium/Low carry-forward in §2.3.                                            |
...
- [x] `providers/gemini.py` + `providers/ollama.py` removed; `google-generativeai` dropped from `pyproject.toml`. Legacy `type=gemini|ollama` maps through the D6 shim (gemini → `…/v1beta/openai`, corrected 2026-07-05; ollama → `:11434/v1` keyless). _Live Gemini/Ollama validation over the OpenAI-compatible endpoints (`requires_cloud`/`requires_ollama`) still pending on a real machine (§2.3 carry-forward)._
...
- [x] **Model discovery** (`providers/discovery.py`): on-demand `GET /v1/models` POST (Test + project "Load models") yields the model list/count, **never on render, no background thread**. Persisted to a workspace JSON cache (`core/connection_models.py` → `~/.weaver/connection_models.json`) with a `(stale)` hint after the TTL; the Switch AI datalist reads the selected connection's cached models (a single grouped-by-connection picker across all connections remains a nice-to-have).
...
- [x] `services/routing.py` precedence (`[routing.<task>]` → legacy `[provider]` (when it carries a model) → workspace `[defaults].default_connection`) wired into `translate_project`. Simple per-segment fallback (try-next on `ProviderError`, 30 s cold-mark) implemented in `translate_one_segment`.
...
- [x] **E1+E2 — Detection gate + bounded repair** (`services/enforcement.py`): `evaluate_translation` reuses the QA primitives (glossary/character/untranslated-JP) + a loose anti-truncation floor; on violation, **one** bounded repair re-ask (`[translation] enforce_repair`, default on), re-validated, committed only if not strictly worse — never blocks, never silently substitutes.
- [x] **E3 — `[translation_profile]` style contract**: `<profile>` prompt block (`tone`/`dialog_style`/`name_rendering`/`tense`) + deterministic `banned_phrases` soft check (seed `core/slop_seed.py`; `[]` disables, custom array replaces).
- [x] **E4 — `uncertain_terms` → discovered glossary candidates** (`storage.record_uncertain_glossary_candidate`, idempotent, `category="discovered"`, no extra model call).
- [x] Gates green per slice (E1–E4). _Owner-visible E2 repair token-cost delta on a live run still pending (§2.3 carry-forward)._
