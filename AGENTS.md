...
>
> **Current Phase:** **v0.7.3 — Performance & Reliability Release (in progress; M1+M2 done).** v0.7.2 is tagged; audit blockers H1–H3 merged via PR #56. Sources of truth: [execution plan 2026-07-05](docs/superpowers/specs/2026-07-05-v073-performance-execution-plan.md) + [ADR 020 draft](docs/decisions/020-bounded-translate-concurrency.md) (Proposed) + **[measured audit 2026-07-13](audit_weaver_073.md)** (verifies F1–F8/A1–A8 against current code and adds findings **N1–N8** with real measurements — absorbed into the §2.3 milestones).
>
> **Status:** **M1+M2 complete, neither merged/pushed.** M1 on `perf/v073-storage-quick-wins` (`ef3bbe3`/`fe4a1c2`/`eeb63ee`); **M2 on `fix/v073-audit-carryforward`** (stacked on M1; `631ce46`…, one commit per slice) closes A1/A2+A7/A3/A8 + N3/N4/N5. Gates green 2026-07-14 (ruff / format / pyright clean; **full suite 1653 passed, 1 skipped**; bench all budgets PASS incl. flat cost **1.04×**; acceptance gate AC-1…AC-9 PASS). Schema still **v13**. ADR 020 must be Accepted before M4 (concurrency). **Next milestone: M3** (`feat/v073-enforcement-provenance`, migration v14). Merge order: M1 PR first, then M2 PR.

> **Current Objective:** Execute the v0.7.3 milestones in order (M1 bench-baseline + storage/algorithmic quick wins ✅ → M2 reliability + input-safety carry-forward ✅ → **M3 enforcement provenance + honest cost accounting (next)** → M4 bounded concurrency → M5 validation/release gate). One milestone = one branch = one PR.
>
...
| **M3 — Enforcement provenance + honest cost accounting** (P1, `feat/v073-enforcement-provenance`) | Detection ungated from `enforce_repair` (A4a); verdicts/repair outcomes persisted (migration **v14** — M1.5 took v13 for `idx_glossary_candidates_project`, columns on `translations`) + row/summary token reconciliation (A4b+A5); explicit `max_retries` on the OpenAI client **and** resolve the dead `[translation] max_retries` init key — wire it or delete it (F6+N2, `services/project.py:350`); repair/JSON-repair call counts in the run summary. **Grouped accounting accuracy:** CJK-aware token estimator (N7 — `chars//4` undercounts JP ~3×; two-class heuristic, no new dependency) + recalibrate `MAX_CONTEXT_TOKENS` afterwards. | 📋 |
...
**Next:** M1+M2 done — start **M3** on `feat/v073-enforcement-provenance` (migration v14). M4 may slip to v0.7.4 without breaking the release (decide at M3 exit).
...
- [ ] **Reliability:** ~~corrupt `connections.toml`/`secrets.toml` is never silently destroyed (backup-before-rewrite tested)~~ ✅ M2.1; ~~dead primary with a healthy fallback completes the run~~ ✅ M2.2 (e2e-tested); enforcement verdicts/repair outcomes persisted + surfaced — **open, M3**.
...
- [ ] **Accuracy accounting:** per-segment row tokens reconcile exactly with the run summary incl. repair — **open, M3**; ~~legacy brand recorded in attempt history again~~ ✅ M2.3; ~~`weaver inspect` routing-aware~~ ✅ M2.4; `[translation] max_retries` wired-or-deleted (N2) + token estimator CJK-aware + `MAX_CONTEXT_TOKENS` recalibrated (N7) — **open, M3**.
...
| Perf & bench (v0.7.3 M1) | `docs/superpowers/handoffs/2026-07-13-v073-m1-storage-quick-wins.md` · commits `ef3bbe3`/`fe4a1c2`/`eeb63ee` | **mtime cache keys must NOT include the `-wal` mtime** — a read-only WAL read creates/touches `-wal`, so a wal-sensitive key spuriously misses on the absent→present transition; key on `(project.toml, weaver.db)` mtime + TTL only. Prove cache/N+1 wins with **counting-seam tests**, not weak `state=="ready"` assertions. Scope O(n²) rolling-window CTEs to the chapter (pin with an inline old-SQL oracle). `weaver validate` exits 1 when `critical`>0, so the bench fake pattern must be pure-English. Never trust a bench "exit 0" read through a `\| tee`/`\| tail` pipe. |
