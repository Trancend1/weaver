...
>
> **Current Phase:** **v0.7.3 — Performance & Reliability Release: SHIPPED, tagged `v0.7.3` 2026-07-27 with live validation formally Risk Accepted (§2.4).** v0.7.2 is tagged; audit blockers H1–H3 merged via PR #56. Sources of truth: [execution plan 2026-07-05](docs/superpowers/specs/2026-07-05-v073-performance-execution-plan.md) + [ADR 020](docs/decisions/020-bounded-translate-concurrency.md) (**Accepted 2026-07-21**) + **[measured audit 2026-07-13](audit_weaver_073.md)** (verifies F1–F8/A1–A8 against current code and adds findings **N1–N8** with real measurements — absorbed into the §2.3 milestones).
>
> **Status:** **M1–M4 are merged to `main`** (M4 via PR #60, merge `553ebc5`; verified 2026-07-26: `main == origin/main`, 0/0 divergence). Schema is **v14**. **M5 is complete for everything reachable offline** on `chore/v073-release-gate` — version bumped to **0.7.3** (pyproject + `__init__.py` + `tauri.conf.json`), CHANGELOG cut, two gated live tests authored, and two latent release bugs fixed (see below). Gates green 2026-07-26: ruff / format / pyright clean; **full suite 1733 passed, 1 skipped, 3 deselected**; bench all budgets PASS incl. **concurrency scaling 2.78×** and flat cost 0.74×; acceptance gate AC-1…AC-9 PASS; wheel `weaver-0.7.3-py3-none-any.whl` builds with all web/prompt assets; web smoke confirms 127.0.0.1-only binding and `/version` → `0.7.3`. **M5 merged via PR #61** (merge `92e9470`) and **re-verified on `main` 2026-07-27** — full suite exit 0 **1733 passed / 1 skipped / 3 deselected**, all bench budgets PASS (concurrency **2.64×**, flat cost 0.72×), AC-1…AC-9 PASS, migration v14 forward + idempotent on a fresh DB, all three version sources `0.7.3`. **Tagged `v0.7.3` on 2026-07-27 with live validation Risk Accepted** — the owner waived §2.4's live criteria after being shown they were unmet; exposure/mitigation/rollback are recorded in that §2.4 entry. Handoff: [2026-07-26](docs/superpowers/handoffs/2026-07-26-v073-m5-release-gate.md).

> **Current Objective:** v0.7.3 is **shipped** (M1 ✅ → M2 ✅ → M3 ✅ → M4 ✅ → M5 ✅, tag `v0.7.3`). **Carry-forward debt into v0.7.4:** run the four waived live validations (§2.4) — the gated tests are written and waiting — and fix forward from whatever they find. Then open the **Cross-Platform Desktop (macOS/Linux)** sprint with a fresh plan + ADR (§2.1.1).
>
...
| **M4 — Bounded translate concurrency** (P1, `feat/v073-bounded-concurrency`) | Opt-in `[translation] max_concurrent = 1..4` (default 1 = today bit-for-bit); FakeProvider latency knob; per-worker connections; locked cold-mark; CLI `--max-concurrent` + cockpit 1–4 select; bench scaling budget. | ✅ **Done** (`337e82d`…`2fc8f5d`, 21 commits; not merged/pushed). ADR 020 **Accepted**. New `services/segment_runner.py` is the sole home of translate-loop thread code; both segment loops delegate to it and `batch_translate` inherits via the chapter plan. Two deviations from plan: `ThreadPoolExecutor` replaced by a hand-rolled worker pool (sqlite3 `check_same_thread` forbids closing a connection from the coordinator thread — proven by running the old runner against real connections); and `transaction()` moved to **`BEGIN IMMEDIATE`**, load-bearing because `record_translation()` opens with `SELECT MAX(attempt)` (read-then-upgrade → `SQLITE_BUSY` bypassing `busy_timeout`). Handoff: [2026-07-21](docs/superpowers/handoffs/2026-07-21-v073-m4-bounded-concurrency.md). |
| **M5 — Validation & release gate** (P0, `chore/v073-release-gate`) | Bench + acceptance gate with before/after evidence vs the measured baseline above; live Gemini (`…/v1beta/openai`, current model — `gemini-1.5-flash` retired) + first `requires_ollama` test; E2 repair token-cost delta; concurrency live spot-check; manual `epubcheck` on export artifacts (audit rec — dev-tooling, no runtime dependency); optional py-spy session on `weaver serve` during a batch run (memory-leak evidence gap); CHANGELOG/version/tag `v0.7.3`. | ✅ **Done — merged (PR #61) and tagged `v0.7.3`.** Live half **Risk Accepted** by the owner 2026-07-27, not satisfied (§2.4). Full gate green (numbers in §2.3 Status). Both gated live tests **authored** (`test_gemini_live.py`, `test_ollama_live.py` — first `requires_ollama` usage ever) and the retired `gemini-1.5-flash` shim default replaced with `gemini-2.5-flash`, but **none were run**: owner deferred live validation on 2026-07-26 (no Gemini key, no Ollama daemon, no Java/epubcheck, no py-spy). **Two latent release bugs found and fixed by the gate itself:** (1) `src/weaver/__init__.py` was stuck at `0.7.0`, so `weaver --version` and `GET /version` under-reported through **both** v0.7.1 and v0.7.2 — no test or release check compared it to `pyproject.toml`; now guarded in `tests/unit/test_version.py` **and** `check-version.ps1`. (2) `core.hooksPath` pointed at `.git/hooks` (samples only), so the **mandatory** secret-scan + commit-msg hooks had been inactive; re-pointed at `.githooks` and proven by staging a fake key (exit 1). Also: AGENTS.md had drifted two milestones behind CLAUDE.md — re-synced. |
...
**Next:** M1–M4 merged; M5 offline-complete on `chore/v073-release-gate` (PR open, **do not tag**). The release is blocked only on checks that need the owner's machine and credentials, in this order:

1. `export GEMINI_API_KEY=…` then `uv run pytest -m requires_cloud -q` — proves the `…/v1beta/openai` endpoint fix **and** the new `gemini-2.5-flash` shim default (the retired `gemini-1.5-flash` would 404).
2. `ollama serve` + `ollama pull qwen3:14b`, then `uv run pytest -m requires_ollama -q` — first-ever run of that marker.
3. Live concurrency spot-check at `--max-concurrent 2` against a real endpoint: needs **≥ 2×**. The 2.78× bench figure is *simulated* latency, not network, so it does not satisfy this.
4. E2 repair token-cost delta on a live run (M3 made it visible) → confirm or flip the `enforce_repair` default.
5. Optional/deferred dev-tooling: `epubcheck` on an exported EPUB (needs a JRE), py-spy session on `weaver serve` during a batch run.
6. Then, and only then: `git tag v0.7.3` + push (CI's `check-version.ps1 -Tag` will verify all three version sources agree).

### 2.4 Exit Criteria

#### v0.7.3 Performance & Reliability exit — 📋 OPEN (defined 2026-07-05; tightened by the 2026-07-13 audit)

v0.7.3 ships only when every criterion below has concrete evidence (bench output, test run, or owner-confirmed live check) recorded in the milestone handoffs. **Outcome: 8 of 9 met with evidence; the 9th (live validations) was formally Risk Accepted by the owner on 2026-07-27 and `v0.7.3` was tagged anyway — see that entry for exposure, mitigation, and rollback.**
...
- [x] **Throughput (bench half)** — M4: `max_concurrent = 3` measures **2.77×** on the latency-simulating FakeProvider bench budget (`translate concurrency scaling`, target ≥ 2.4×, 24 segments @ 0.3 s: 14.80 s sequential vs 5.35 s). ⏳ **Live spot-check (≥ 2× against a real provider) still open — M5.**
...
- [x] **No regressions** — ✅ **M5 full re-run 2026-07-26** (`chore/v073-release-gate`, after the version bump + new tests): ruff / format (395 files) / pyright **0 errors** clean; full suite exit 0, **1733 passed / 1 skipped / 3 deselected** (skip = POSIX file mode on Windows; deselects = the 3 live tests); **all bench budgets PASS** (concurrency **2.78×**, flat cost **0.74×**, translate 0.68 ms/seg); **AC-1…AC-9 PASS**; wheel `weaver-0.7.3-py3-none-any.whl` ships 71 templates + 4 static + `schema.sql` + all 3 prompt templates; web smoke `/healthz` 200, `/version` → `0.7.3`, LAN interface refused, no orphan process; shared core free of web/CLI framework imports; no key-shaped literals tracked. Migration v14 forward-tested + idempotent ✅ M3. _(Prior M4 evidence 2026-07-21: **1731 passed / 2 skipped**, all budgets PASS incl. concurrency 2.77× and flat cost 0.90×, AC-1…AC-9 PASS. Gate B1 re-checked in M4: `transaction()` requires a writable connection and every render path uses `connect_readonly_database`, so the `BEGIN IMMEDIATE` change cannot reach a GET handler.)_
- [ ] **Live validations — ⚠️ RISK ACCEPTED by the owner 2026-07-27; `v0.7.3` shipped without them.** This criterion is **NOT met**; it is knowingly waived, not satisfied. Do not re-render it as green. M5 did everything short of execution: the Gemini `requires_cloud` test and the first-ever `requires_ollama` test are written and collected, and the retired `gemini-1.5-flash` shim default is replaced with `gemini-2.5-flash`. Unrun because the environment lacks the credentials/daemons (re-probed 2026-07-27, unchanged): no `GEMINI_API_KEY` (shell env or `~/.weaver/secrets.toml`), no `ollama` binary and nothing on `:11434`, no `java`/`epubcheck`, no `py-spy`. Outstanding: (a) Gemini gated test passes live; (b) Ollama gated test passes live; (c) live concurrency spot-check ≥ 2× at `--max-concurrent 2` — **the bench figure does not count, it is simulated latency**; (d) E2 repair token-cost delta recorded → `enforce_repair` default confirmed. Both live tests deliberately **fail rather than skip** once opted into, so a green run is real evidence (§4.3 gate 5).
  - **Exposure:** every live provider path in v0.7.3 is unproven against a real endpoint. Highest-probability failure is the `gemini-2.5-flash` shim default (an untested constant, though strictly better than the retired id it replaces); next is the `…/v1beta/openai` base_url, which has never once served a live request.
  - **Mitigation:** the default `[translation] max_concurrent = 1` keeps concurrency off unless opted in, so the least-tested new code path is not on by default; `openai_chat` is a single brand-free transport, so a bad endpoint surfaces as an actionable `ProviderUnavailable` rather than corrupt output; connections are user-configurable, so a wrong shim default is overridable without a release.
  - **Rollback:** `git push --delete origin v0.7.3 && git tag -d v0.7.3`, then `gh release delete v0.7.3 --yes` (the tag push triggers `.github/workflows/release.yml`, which publishes a GitHub Release with the NSIS installer). No schema or data rollback is needed — migration v14 is forward-only and additive, and nothing in v0.7.3 rewrites existing rows.
  - **Trigger to close:** run the four checks above on the owner machine; if any fails, fix forward in **v0.7.4** rather than un-publishing, unless the failure makes translation unusable for every provider.
...
| Bounded concurrency (v0.7.3 M4) | `docs/superpowers/handoffs/2026-07-21-v073-m4-bounded-concurrency.md` · ADR 020 · commits `337e82d`…`2fc8f5d` | **A sqlite3 connection may only be closed by the thread that created it** (`sqlite3.connect()` defaults to `check_same_thread=True`) — so a thread pool that hands connections back to a coordinator to close is unusable; each worker must own and close its own. **`transaction()` must use `BEGIN IMMEDIATE`:** under DEFERRED, a transaction whose first statement reads (e.g. `record_translation()`'s `SELECT MAX(attempt)`) takes a SHARED lock then races to upgrade, and SQLite reports that upgrade race as instant `SQLITE_BUSY` **without ever consulting `busy_timeout`** — measured 121/160 failures in 0.055 s vs 0/160 after. **Prove concurrency tests are load-bearing by mutation**, never by green: a "batch inherits max_concurrent" test asserting only identical counters passes even when the hand-off is missing entirely (it was — `batch_translate` builds `TranslationPlan` directly, not via `prepare_chapter_translation`). Validate cheap inputs **before** the expensive step (provider healthcheck, dry-run early return) or a bad flag surfaces as an unrelated failure. Under a worker window a segment failure does **not** halt dispatch — later segments still commit real writes, and the raise discards the summary, so the database is the only source of run state. |
| Release gating (v0.7.3 M5) | `docs/superpowers/handoffs/2026-07-26-v073-m5-release-gate.md` | **A version constant with no cross-file assertion will drift, and self-referential tests cannot see it.** `weaver.__version__` sat at `0.7.0` across two releases while `pyproject.toml` moved to `0.7.2`; every existing test compared `__version__` to itself (`assert weaver.__version__ in result.output`), so all stayed green. The guard must parse the *other* file (`tests/unit/test_version.py` reads `pyproject.toml`; `check-version.ps1` covers pyproject ↔ tauri ↔ `__init__.py`) — and prove it by watching it fail before the bump. **Verify tooling is actually wired, not merely present:** `.githooks/` existed and was correct, but `core.hooksPath` pointed at `.git/hooks` (samples only), so the mandatory secret-scan had never run — check `git config core.hooksPath` and prove the hook by staging a fake key. **A doc mirror silently rots:** AGENTS.md sat two milestones behind CLAUDE.md because nothing compares them; re-sync (`Compare-Object`) as part of the release gate. Bench/gate exit codes must be captured to a file, never read through a pipe. See the `rtk` output-fidelity row below — it nearly corrupted this milestone's own evidence twice. |
| **Shell tooling — `rtk` truncates output silently** | This session (v0.7.3 M5 + the 0.7.1/0.7.2 changelog reconstruction) | **`rtk` is the default command layer (`RTK.md`) but it is a lossy filter, and it does not tell you when it drops data. Never let an enumeration that a decision depends on come out of `rtk`.** Three failure shapes, all observed: (1) **silent truncation with a self-contradicting count** — `git log v0.7.0..v0.7.1 --no-merges --format='%s' \| grep -cE '^(feat\|fix)'` printed `95` while the very same pipeline displayed only **15** lines; reading those 15 would have concluded v0.7.1 held 15 changes instead of 95. (2) **truncation survives redirection** — `git log … > file` then grepping the file produced false negatives (6 of 8 real commit-subject claims reported MISS); it was not a display limit. (3) **builtins mangled or unavailable** — `diff` failed with `rtk: The system cannot find the path specified`, `git branch -vv` echoed the literal string `git branch`, and `git log --oneline` silently omitted merge commits (hid merge `553ebc5`, making a merged branch look unmerged). **Rule:** for counts, full listings, file comparisons, or any claim you will write into a doc/commit/report, use `PowerShell` (`Compare-Object`, `Where-Object`, `$x.Count`) or capture to a file and read it with the Read tool. Cross-check any count against the number of lines actually shown — if they disagree, the output is truncated, not the repo. `rtk` stays fine for its intended job: compact status/diff/grep while orienting. |
