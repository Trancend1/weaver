...
**Track(s) active:** branch `feat/connection-first-routing`. **Bagian A done** (engine de-brand behind D6 shim). **Bagian B done**: connection-first cockpit (registry + discovery + Add/Test/Delete, "Connections" surface) + model-centric project AI (Active AI + Switch AI + routing resolver in the translate pipeline). **S1 done** (collapse to `openai_chat`+`fake`, natives + `google-generativeai` removed, keyless edge fixed — verified). **S2 done** (`[defaults]` tier + simple per-segment fallback with cold-mark, rescue tested). All green: ruff/pyright clean. **Remaining:** S3 (discovery cache + grouped picker), S4 (cockpit cleanup + fallback-author UI), S5 (CLI parity) — see §2.4 breakdown. Gated by T6/T7/T8.
