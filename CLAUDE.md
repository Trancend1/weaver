...
> **Status (2026-06-08):** **`v0.7.0` stable.** Phases A–F complete. **Sprints G–K complete.** Strategic pivot (ADR `009`): post-Phase-F roadmap is Sprint G–O on the line "HTMX-first, FastAPI-stable, Tauri-sidecar-ready". The earlier "Phase Final — npm `@weaver/cli` wrapper" target is **deferred legacy** — replaced by a Tauri sidecar path that hardens the FastAPI runtime first. **Active sprint: Sprint L — Candidate Review + Character Text Draft.** Full plan: [docs/weaver_next_plan.md](docs/weaver_next_plan.md). Governing ADRs: `009` (strategic pivot), `010` (persistent job core), `011` (Project terminology). Sprint K gate (last green): **981 tests / 4 skipped**, pyright 0, ruff + format clean, clean wheel build.
...
| [docs/weaver_next_plan.md](docs/weaver_next_plan.md) | **Active post-Phase-F roadmap: Sprint G–O (HTMX-first, FastAPI-stable, Tauri-sidecar-ready). Governed by ADR `009`. Sprints G–K complete; Sprint L (Candidate Review + Character Text Draft) is the active sprint.** |
...
| Sprint J — EPUB Preservation Snapshot & Parser Hardening | Persist Phase F `ParsedEpub` into 6 additive tables keyed by `volume_id`; source-hash + parser-version invalidation; reparse-as-Job (consumes Sprint I); image-format dimension coverage (JPEG/WebP/SVG); CLI `weaver epub-inspect`. Schema v4 → v5. | ✅  |
| Sprint K — Export Fidelity Integration | Renderer reads preservation snapshot; pre-export advisory in preflight; post-export fidelity report on job result; regression gate `epub_export_fidelity`; atomic export write (`.partial` → rename). | ✅ |
...
### 2.3 Active Phase — Sprint L: Candidate Review + Character Text Draft

> Sprints G–K are complete. The active sprint is **Sprint L** (Candidate Review + Character Text Draft). Full scope and task order live in [docs/weaver_next_plan.md](docs/weaver_next_plan.md) Sprint L section. Sprint M may **not** start until Sprint L's final gate passes; run the §2.2 phase gate before each sub-stage.

**Sprint L — in-scope summary:**

- **Translation candidate model** — new schema entity tracking proposed alternative translations with status state machine (`proposed` / `accepted` / `rejected` / `superseded`).
- **Grounded candidate generation** — explicit diff context presented to the user; no auto-mutation of active translations.
- **Character Page text draft** — XHTML/text-only draft page (no images/OCR) with provenance stamp on every AI-generated artifact.
- **Schema v7 → v8** — additive tables for candidates and character page drafts.

**Sprint L — out of scope (deferred to later sprints per ADR `010`/`012`):**

- Image/OCR pipeline (Sprint M).
- Celery / Redis / RQ / external worker. The `src/weaver/api/jobs.py:8-10` boundary stays in force.
- EPUB preservation or export changes (Sprints J, K are done).
...
**Sprint L — exit criteria** (governed by ADR `010`/`012`; full task order in [docs/weaver_next_plan.md](docs/weaver_next_plan.md)):

- [ ] L1 — Schema v7 → v8 migration ships: additive tables for translation candidates and character page drafts. Forward-only, idempotent, regression-tested.
- [ ] L2 — Translation candidate model with `proposed` / `accepted` / `rejected` / `superseded` state machine; grounded generation with diff context; no auto-mutation of active translations.
- [ ] L3 — Character Page text draft: XHTML/text-only (no images/OCR); provenance stamp on every AI artifact (model, prompt, timestamp, source segments).
- [ ] L4 — All new code follows Sprint I job pattern; cold-start recovery marks `running` → `failed/process restart`; no auto-resume.
- [ ] L5 — Final gate: full test suite green, pyright 0, ruff + format clean, clean wheel build, readiness report appended to §2.5.
...
| Sprint K — Export Fidelity Integration | **Complete (branch `feat/export-fidelity`).** Five stages K1–K5. K1 snapshot-aware preflight (`ui_qa.py` `_snapshot_export_advisories`) — missing/stale/error badges on EPUB export; K2 renderer preservation — OPF metadata, manifest/spine, CSS/fonts/images preserved per Sprint J; K3 atomic write (`renderers/_atomic.py` + renderer modifications for `os.replace` workflow, no write leak); K4 post-export fidelity report (`EpubExportFidelityReport`, `epub_export_fidelity.py`, `ExportResult.fidelity_reports`, UI fidelity section in `_export_job.html`/`job_detail.html`); K5 8 new tests. Bugfix: `BaseException`→`OSError` in `_atomic`, moved `logger` after imports in `export_book.py` (E402), `compute_source_hash` to top import in `ui_qa.py`, relative `source_path` resolution in preflight, removed unused `EpubWriteError` import. Gate: **981 passed / 4 skipped** (was 973/4; +8 new tests), pyright 0, ruff + format clean, clean wheel build. |
...
- **Sprint L is candidate-review + character-draft only** (ADR `010`/`012`): no image/OCR endpoints, no EPUB preservation/export changes, no external queue — `src/weaver/api/jobs.py:8-10` remains in force.
