...
Sprint status: Sprints 1–3 complete (novel model + multi-format import; FastAPI cockpit foundation; translation workspace read/save/history). **Sprint 4A+4B complete** — 4A: `POST …/chapters/{id}/translate` + `…/translate-segments` start background jobs, `GET …/jobs/{job_id}`, per-request provider/model override, skip-already-translated. 4B: per-segment progress snapshot + enriched job status, cooperative cancel (`POST …/jobs/{job_id}/cancel`), optional SSE stream (`GET …/jobs/{job_id}/events`); single-process thread worker only (no external queue). Next: **Sprint 4C** (safe retranslate, overwrite protection, preserve manual edits).
