...
| 11 | FastAPI UI functional parity (ADR `007`: Jinja2+HTMX, presentation-only, reuse Sprint 2–10 APIs) — 11A shell ✅ · 11B-1 create/import ✅ · 11B-1.5 import-collision fix ✅ · 11B-2 workspace read/save/history ✅ · 11B-3 translate/export ⏳ · 11C glossary/character/TM/config. **Functional parity, not visual polish.** | 7/10 | 🟡 Medium | 🟡 |
...
- Backend direction is **FastAPI-first** (ADR `004`) — but **Flask stays the working baseline** until UI parity. Do not delete Flask; no default-`serve` flip yet.
...
- **Sprint 11 (active) — FastAPI UI functional parity:**
  ...
  - **11B-3 translate/retranslate + jobs + export → next stage**
  - 11C glossary/character/TM/provider config — pending
...
**Validation baseline (Gate 11B-2):** 655 tests / 4 skipped · pyright 0 · ruff + format clean · Flask 13 routes (`/`→200) · FastAPI 63 route objects · CLI 15 commands.
