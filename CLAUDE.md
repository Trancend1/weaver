...
>
> **Current Phase:** **v0.7.2 — Connection-First Routing (ADR 018) + Translation Enforcement Loop (ADR 019)**, on branch `feat/connection-first-routing` (owner direction: complete everything in 0.7.2, **no PR**). Desktop Installer & Release Hardening (ADR 017, v0.7.1) is the shipped baseline.
>
> **Status:** ADR 018 done (S1–S5) + post-S5 hardening done (cockpit/glossary/candidate routing-aware, dup CTA dropped, copywriting pass); full suite **1553 passed**. ADR 019 (enforcement loop + anti-slop) accepted, **E1–E4 in progress** (§2.3/§2.4). Tag v0.7.2 when E1–E4 land. See §2.3 for the live detail.

> **Current Objective:** Make glossary/character/TM **binding, not decorative**, with a bounded verify→repair loop + anti-slop contract (ADR 019), on top of the shipped connection-first routing — all under v0.7.2, hard rules (ADR 018 D9 / Gate B1) intact.
>
> **Current Sprint:** v0.7.2 (active, §2.3). Next sprint after tag: **Cross-Platform Desktop (macOS/Linux)** with a fresh plan + ADR (§2.1.1).
...
| **v0.7.2 — Connection-First Routing + Enforcement Loop** | 🟡 In progress    | ADR 018 (done) + ADR 019 (accepted, in progress). **018:** one `openai_chat`(+`fake`), removed gemini/ollama natives + `google-generativeai`, connection registry + per-segment routing/fallback + discovery cache + CLI parity (S1–S5). **Post-S5 (done):** cockpit translate + glossary-suggest + candidate made routing-aware (`resolve_translation_engines`/`resolve_consumer_config`) — fixes live `provider.model missing`/`incomplete`; dup retranslate CTA dropped; copywriting pass. **019 (in progress):** glossary/character/TM enforcement loop + anti-slop (E1–E4). All on `feat/connection-first-routing`, **no PR**; tag v0.7.2 when E1–E4 land. Active phase §2.3.                                                                                                                                                                                                                                                             |
...
### 2.3 Active Phase — v0.7.2: Connection-First Routing (ADR 018) + Translation Enforcement Loop (ADR 019)

> **Status (2026-06-16):** v0.7.2 bundles three workstreams on branch `feat/connection-first-routing` (owner direction: **complete everything in 0.7.2, no PR**). **(1) Connection-First Routing (ADR 018) — done** (Bagian A+B, S1–S5). **(2) Post-S5 hardening — done:** cockpit translate path made routing-aware (chapter + batch planners + glossary-suggest + candidate-generation all honored `[provider]` only → now resolve `[routing.*]`/Active AI; fixes the live "provider.model missing" / "Provider configuration is incomplete" errors); duplicate `skip_existing` retranslate CTA removed; plain-language copywriting pass across the cockpit. **(3) Translation Enforcement Loop (ADR 019) — accepted, in progress:** make glossary/character/TM *binding* not decorative + anti-slop. Full suite green at each step (1553 passed). **Remaining:** implement ADR 019 E1→E4 (see §2.4), then tag v0.7.2. Deferred-by-design (D9) stays out: circuit breaker, health-score, presets, cost dashboard, rotation window, `routing_decisions` ledger.
>
> **Deferred-but-tracked:** `weaver inspect` (`services/project.py:244`) is the **last** direct `[provider]` reader (CLI display + `--healthcheck`); left as a routing seam per owner, not a cockpit blocker. Live Gemini/Ollama validation over OpenAI-compatible endpoints (`requires_cloud`/`requires_ollama`) still pending on a real machine.
...
**Track(s) active:** branch `feat/connection-first-routing`. **Bagian A done** (engine de-brand behind D6 shim). **Bagian B done**: connection-first cockpit (registry + discovery + Add/Test/Delete, "Connections" surface) + model-centric project AI (Active AI + Switch AI + routing resolver in the translate pipeline). **S1 done** (collapse to `openai_chat`+`fake`, natives + `google-generativeai` removed, keyless edge fixed — verified). **S2 done** (`[defaults]` tier + per-segment fallback with cold-mark, rescue tested). **S3 done** (workspace model-discovery JSON cache + cache-only render + stale hint). **S4 done** (legacy editor reframed + Add/Clear fallback UI backed by a section-rewriting `set_routing`). All green: ruff/pyright clean. **Remaining:** **S5** (CLI parity: `weaver connections …`, `weaver routing …`) — see §2.4 breakdown. Gated by T6/T7/T8.
...
- [x] `providers/gemini.py` + `providers/ollama.py` removed; `google-generativeai` dropped from `pyproject.toml`; `gemini_generate`/`ollama_generate` protocols gone (registry = `openai_chat` + `fake` only). Legacy `type=gemini|ollama` auto-maps to an `openai_chat` connection via the D6 shim (Gemini → `…/v1beta`, Ollama → `:11434/v1` keyless). Keyless `openai_chat` (empty `api_key_env` → dummy key) added + tested. (S1, verified 2026-06-16: ruff/pyright clean, pytest 1519 passed.)
...
#### Translation Enforcement Loop (ADR 019) exit — 🔜 IN PROGRESS (v0.7.2)
...
- [ ] **E1 — Detection gate (free, always-on).** New `services/enforcement.py` (pure) reusing `check_glossary_mismatch` + `check_character_name_missing` + JP-residue + a loose anti-truncation floor; wired into `translate_one_segment` pre-commit; violations recorded + surfaced. **No model call, no repair yet** — this alone kills the gimmick. `[qa] minimum_length_ratio` is NOT a trigger (forcing length causes padding-slop).
- [ ] **E2 — Targeted repair re-ask (bounded 1-pass, switchable).** New `enforcement_repair.jinja2`; `repair_once(provider, …)` (takes `provider` param — `[routing.repair]` seam, deferred). `[translation] enforce_repair` default true; off → detection-only. Repass tokens counted/shown.
- [ ] **E3 — `[translation_profile]` contract + banned-slop seed.** Pure config; emit `<profile>` block; small override-able seed in `core/slop_seed.py`, soft trigger, active only when profile declared.
- [ ] **E4 — Recover discarded `uncertain_terms` → `glossary_candidates`** (`frequency++`, no extra model call; replaces the proposal's "continuous extraction").
- [ ] `ruff`/`ruff format`/`pyright`/`pytest` green each slice; owner-visible token-cost delta before locking E2 default-on.
