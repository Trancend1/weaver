...
| **Connection-First Routing (v0.7.2)**                     | 🟡 Impl. complete | ADR 018. Collapsed to one `openai_chat` protocol (+`fake`); removed `gemini.py`/`ollama.py` + `google-generativeai`; de-branded engine/cockpit; `model` = open string; workspace connection registry + check + per-segment routing/fallback + discovery cache + CLI parity (S1–S5). On `feat/connection-first-routing`, awaiting owner review/merge. Active phase §2.3.                                                                                                                                                                                                                                                             |
...
### 2.3 Active Phase — Connection-First Routing (v0.7.2) 🟡 IMPLEMENTATION COMPLETE (ADR 018)

> **Status (2026-06-16):** All planned work landed on branch `feat/connection-first-routing` — Bagian A + B and S1–S5 (see §2.4). ruff/pyright clean; full suite green each slice. **Next:** owner review of the branch + live validation of Gemini/Ollama over their OpenAI-compatible endpoints (`requires_cloud`/`requires_ollama`), then merge + tag. Deferred-by-design (D9) stays out: circuit breaker, health-score formula, presets, cost dashboard, rotation window, `routing_decisions` ledger.
