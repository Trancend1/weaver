...
>
> **Current Phase:** **v0.7.3 — Performance & Reliability Release (in progress; M1+M2+M3+M4 done).** v0.7.2 is tagged; audit blockers H1–H3 merged via PR #56. Sources of truth: [execution plan 2026-07-05](docs/superpowers/specs/2026-07-05-v073-performance-execution-plan.md) + [ADR 020](docs/decisions/020-bounded-translate-concurrency.md) (**Accepted 2026-07-21**) + **[measured audit 2026-07-13](audit_weaver_073.md)** (verifies F1–F8/A1–A8 against current code and adds findings **N1–N8** with real measurements — absorbed into the §2.3 milestones).
>
> **Status:** **M1–M3 are merged to `main`** (verified 2026-07-21: `main == origin/main`, 0/0 divergence; `git branch --merged main` lists `perf/v073-storage-quick-wins`, `fix/v073-audit-carryforward`, `feat/v073-enforcement-provenance`). Schema is **v14** (enforcement provenance columns on `translations`). **M4 is complete but not merged and not pushed** — `feat/v073-bounded-concurrency`, 21 commits `337e82d`…`2fc8f5d`, ADR 020 Accepted in `337e82d`. Gates green 2026-07-21: ruff / format / pyright clean; **full suite 1731 passed, 2 skipped**; bench all budgets PASS incl. **concurrency scaling 2.77×** (target ≥ 2.4×) and flat cost 0.90×; acceptance gate AC-1…AC-9 PASS. **Next: open the M4 PR, then M5 (`chore/v073-release-gate`) for the live validations M4 could not cover offline.** Handoff: [2026-07-21](docs/superpowers/handoffs/2026-07-21-v073-m4-bounded-concurrency.md).

> **Current Objective:** Execute the v0.7.3 milestones in order (M1 bench-baseline + storage/algorithmic quick wins ✅ → M2 reliability + input-safety carry-forward ✅ → M3 enforcement provenance + honest cost accounting ✅ → M4 bounded concurrency ✅ → **M5 validation/release gate**). One milestone = one branch = one PR.
>
...
**Next:** M1–M4 done (M1–M3 merged; M4 awaiting its PR). **M5 (`chore/v073-release-gate`) closes the release** — its remaining work is the live/offline-impossible checks: Gemini over `…/v1beta/openai`, first `requires_ollama` test, E2 repair token-cost delta, a live concurrency spot-check (≥ 2× against a real provider — the 2.77× bench figure is simulated latency, not network), manual `epubcheck`, optional py-spy session, CHANGELOG/version/tag.
