...
| 13 | Flask decommission — only if a flipped default proves stable **and** explicitly approved | 5/10 | 🟡 Medium | ⬜ |
...
- Backend direction is **FastAPI-first** (ADR `004`). **Sprint 12B flipped the default:** `weaver serve` = FastAPI cockpit; Flask kept as `weaver serve-flask` (fallback). Do not delete/deprecate Flask, do not remove the dependency — removal is Sprint 13 (gated + explicitly approved).
...
- **Sprint 13 (next) ⬜** — Flask decommission. Only after the flipped default proves stable **and** is explicitly approved.

**Locked posture (ADR `004` · Gate 12B, 2026-06-03):** `serve` = **FastAPI** cockpit (default UI + API) · `serve-api` = FastAPI headless · `serve-flask` = Flask (legacy fallback). **No Flask removal, no deprecation, no dependency removal** — that is Sprint 13, gated on the flip proving stable + explicit approval.
...
**Validation baseline (Gate 12B, 2026-06-03):** 688 tests / 4 skipped · pyright 0 · ruff + format clean (238 files) · default `serve`→FastAPI cockpit (`/ui`→200) · `serve-flask`→Flask (`/`→200) · FastAPI 53 JSON + 35 UI route entries · CLI 16 commands.
...
**Web cockpit:** **Flask (sync) — legacy working baseline.** **FastAPI — target direction** (ADR `004`), behind optional extra `weaver[web]`; core install pulls no web framework. UI is server-rendered **Jinja2 + HTMX**, no Node/build, no SPA (ADR `007`); HTMX vendored as a static asset (no CDN). asyncio unlocked **only** for the FastAPI web layer. Migration is staged, route-by-route, preserving Flask until parity.

**Still rejected (no reintroduction without ADR):** Django · SQLAlchemy · Celery · RQ · Docker · React/Node build · SPA framework · OpenTelemetry · Sentry. asyncio remains rejected outside the web layer.
